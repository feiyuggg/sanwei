...
var startTerraExplorer4Web = function (divId, SGUrl, cb, options) {
  ...
  function startAfterScripts() {
    ...
    var splashMessagesTypes = {
      UNSECURED_SERVER_TERRAIN:
        "Terrain database from non-secured (HTTP) servers cannot be read <br/> by TerraExplorer for Web when running as a secured website (HTTPS)",
      ...
      LICENSE_INVALID:
        "The SkylineGlobe Server license for this TerraExplorer for Web has expired. Contact the site administrator for further assistance.",
    ...
    function showErrorOnSplashScreen(splashScreenMessage) {
      ...
      $("#loadingDiv").css({ "font-size": "22px", left: left });
    ...
    showProgress("Initializing TerraExplorer for Web...");
    function initTE() {
      TerraExplorer.init();
      ...
      showProgress("Registering Tools...");
      ////////////////////////////////////////////////////////////
      //// Add User's Tools
      var scope =
        typeof window !== "undefined"
    ...
    function onProjectLoaded() {
      if (canContinueFromSplash) {
        ...
        showProgress("Waiting for terrain..");
                    ...
                    Cesium.sampleTerrain(viewer.terrainProvider, 24, [
                      ...
                      TerraExplorer.internal.Navigate.flyToPosition(
                        new TerraExplorer.internal.TEPosition({
                          ...
                          altitudeType:
                            TerraExplorer.internal.TEPosition
              ...
              for (var i = 0; i < $(".menuEntry").length - 1; i++)
                if ($($(".menuEntry")[i]).height() !== $($(".menuEntry")[i + 1]).height()) {
                  if ($('.menuEntry')[i].children[1].textContent !== "")
                    $('#menuButtons span').each(function (i, v) { $(v).css('font-size', '17px') });
                }
